...
今天在独立编程时，发现一个致命性错误，就是在for(初始化语句；判断条件；增量)。初始化在每次程序开始执行一次，能不能进入循环看这个判断条件，循环体跑完才开始执行这个增量。还有就是串口打印的时候，代码在自定义的printf中错误将usart1传入进去，应该是用usartx，因为我在上面的定义就是用usartx来的，修改后串口可以将数据进行一个打印。还有就是用逻辑分析仪抓到了串口的数据，并进行一个分析。 
