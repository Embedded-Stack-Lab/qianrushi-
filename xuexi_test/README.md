...
2026年5月24日
今天将base time的代码进行一个编写，准备用base time来控制led灯的亮度。通过按键来切换LED 的工作模式。定时器首先，先开启时钟配置时基，清除中断标志位，配置中断，使能定时器，配置NVIC启动定时器，核心公式：T=(PSC+1)×(ARR+1)/TIM_CLK，需注意PSC/ARR从0计数，16位上限65535。明天准备调试这个模块。