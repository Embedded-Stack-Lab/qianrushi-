...
=======
2026年1月28日
1.总结昨天晚上的代码通过STM32内部温度传感器+ADC（DMA模式），以1秒为间隔采集芯片温度，通过USART1串口输出至电脑超级终端。
2.1硬件连接：温度传感器集成在芯片内部，与ADCx_IN16通道绑定（实验用ADC1通道16）。
2.2 传感器参数：测量范围-40~125℃，精度±1.5℃左右；需唤醒（默认关闭），采样时间推荐≥17.1μs（实验用55.5个ADC时钟周期）。
2.3ADC基础：12位分辨率，参考电压Vref+=3.3V，数据右对齐存储，采用DMA传输避免CPU占用。
3.1.温度计算公式： 温度(℃) = (V25 - Vsense) / Avg_Slope + 25 ，其中V25（25℃时传感器电压）=1.42V，Avg_Slope（电压-温度斜率）=4.3mV/℃。
4.使能ADC1、DMA1时钟，配置PC1对应GPIO为模拟输入。DMA配置：通道1，循环模式，外设（ADC1_DR）→内存（变量），数据宽度16位。ADC配置：独立模式、连续转换、软件触发，通道11采样周期55.5个时钟，开启自校准。


2026年1月31日
1.今天复习一下ADC是如何配置的代码配置流程：
初始化先根据手册来配置芯片的引脚，因为配置的是ADC1，其挂载在APB2上，先打开时钟，在配置GPIO的引脚。我所使用的是PC1引脚，配置为模拟输入。通过DMA来配置讲数据进行一个快速的传输，先开启时钟再打开复位所有的寄存器，然后配置DMA相关的参数，核心是设置外设地址为ADC1的数据寄存器地址，传输方向为外设到内存，缓冲区大小为1，采用循环模式以实现持续传输。具体参数包括：外设地址不递增，内存地址不递增，外设数据宽度为16位，第一个配置中内存数据宽度设为16位，DMA优先级设为高，并禁用存储器到存储器模式，这些参数旨在实现ADC转换结果的自动传输，在进行一个出书话和打开使能通道。
2.对于一个实时的时钟大致有一个了解， 掉电维持原理：上电时主电源给VBAT供电，掉电后VBAT通过锂电池给RTC供电，数据保存在后备寄存器中。需先使能电源和后备接口时钟（RCC_APB1ENR的PWREN和BKPEN位），再使能后备寄存器访问（PWR_CR的DBP位）。
>>>>>>> 834054a192b6eb8d896c8d812a4bcac93a3c1037
