...
<img width="426" height="283" alt="Image" src="https://github.com/user-attachments/assets/5b76b008-9046-4e18-b0b5-f09c996d246f" />

2.软件设计：ADC和DMA的配置是有ADC1_Init来完成的。在配置完成ADC与DMA后，DMA就开始不停地转换将数值ADC转换的数据进行变换。

3.ADC1_Init()调用了 ADC1_GPIO_Config()和 ADC1_Mode_Config()。这两个函数的作用分别是配置好 ADC1 所用的 I/O 端口；配置 ADC1 初始化及 DMA 模式。
...
1将内部温度转换的代码敲完了，但是感觉温度转换还有问题未解决.

 2026年1月30日 
1.总结昨天晚上的代码通过STM32内部温度传感器+ADC（DMA模式），以1秒为间隔采集芯片温度，通过USART1串口输出至电脑超级终端。 2.1硬件连接：温度传感器集成在芯片内部，与ADCx_IN16通道绑定（实验用ADC1通道16）。 2.2 传感器参数：测量范围-40~125°C，精度±1.5°C左右;需唤醒（默认关闭），采样时间推荐≥17.1μs（实验用55.5个ADC时钟周期）。 2.3ADC基础：12位分辨率，参考电压Vref+=3.3V，数据右对齐存储，采用DMA传输避免CPU占用。 3.1.温度计算公式：温度（°C） = （V25 - Vsense） / Avg_Slope + 25 ，其中V25（25°C时传感器电压）=1.42V，Avg_Slope（电压-温度斜率）=4.3mV/°C。 4.使能ADC1、DMA1时钟，配置PC1对应GPIO为模拟输入。DMA配置：通道1，循环模式，外设（ADC1_DR）→内存（变量），数据宽度16位。ADC配置：独立模式、连续转换、软件触发，通道11采样周期55.5个时钟，开启自校准。 



 2026年1月31日
 1.今天复习一下ADC是如何配置的代码配置流程： 初始化先根据手册来配置芯片的引脚，因为配置的是ADC1，其挂载在APB2上，先打开时钟，在配置GPIO的引脚。我所使用的是PC1引脚，配置为模拟输入。通过DMA来配置讲数据进行一个快速的传输，先开启时钟再打开复位所有的寄存器，然后配置DMA相关的参数，核心是设置外设地址为ADC1的数据寄存器地址，传输方向为外设到内存，缓冲区大小为1，采用循环模式以实现持续传输。具体参数包括：外设地址不递增，内存地址不递增，外设数据宽度为16位，第一个配置中内存数据宽度设为16位，DMA优先级设为高，并禁用存储器到存储器模式，这些参数旨在实现ADC转换结果的自动传输，在进行一个出书话和打开使能通道。 2.对于一个实时的时钟大致有一个了解， 掉电维持原理：上电时主电源给VBAT供电，掉电后VBAT通过锂电池给RTC供电，数据保存在后备寄存器中。需先使能电源和后备接口时钟（RCC_APB1ENR的PWREN和BKPEN位），再使能后备寄存器访问（PWR_CR的DBP位）。



2026年2月1号

1.通常选择 LSI（内部低速时钟）或 LSE（外部 32.768KHz 晶振，精度更高），文档中默认使用 LSI。
2.配置分频系数：RTC_SetPrescaler(32767)（假设时钟源 32.768KHz，分频后 1Hz，即每秒计数 1 次）
3.上程序的理解:上电 → 配系统 + 串口 → 看备份寄存器决定“第一次配置 RTC 还是直接用” → 第一次就初始化 RTC 并设时间，之后只同步+开中断 → 可选把 RTC 时钟输出到外脚 → 清复位标志 → 进入死循环，通过中断和 `Time_Show()` 一直显示当前时间。 
4.程序还是有些不太明白，明天在仔细阅读一下文档。
...
3.如果已经配置过RTC，就只能做同步和中断重启

4.进行条件编译，在rtc时钟输出到Tamper时（调试时调用）

5.清除标志位加进入显示主循环

6.RTC 每秒产生一次中断 → RTC_IRQHandler 里清中断、翻转 LED，并把 TimeDisplay 设为 1
...
今天晚上写了RTC的配置文件，它首先开启电源与备份接口时钟并解除写保护，让 CPU 有权修改位于“安全屋”内的 RTC；接着启动精准的外部低速晶振（LSE）作为 RTC 的心脏；为了防止高速运行的 CPU 指令“噎死”低速运行的 RTC，代码强制加入了等待同步（Wait）机制；最后将 32768Hz 的晶振频率预分频为 1Hz，从而获得精准的“每秒一次”的计时能力。

2026年2月4日

今天晚上将代码完成，extern在 C 语言中，extern 的含义取决于是否有初始化值：extern int TimeDisplay; → 仅声明（告诉编译器这个变量在别处定义）,extern int TimeDisplay = 0; → 被视为定义（实际分配内存
原来的代码问题：在 Dri_RTC.h 中写 extern int TimeDisplay = 0; 时，这个头文件被包含到多个 .c 文件（比如 main.c 和 stm32f10x_it.c），就相当于在每个文件中都定义了一次 TimeDisplay，导致链接时出现"符号重复定义"错误。,还有问题未解决，串口未进行打印。



2026年2月5日

今天晚上学习了iic的部分理论知识，iic分为3种模式标准快速和高速模式，支持一主多从和多主多从是通过仲裁来判断的，stm32 的 I2C 可以使用 DMA 方式操作。一共有两根数据线SDA和SCL，在数据传输时，scl线拉低时sda电平不会改变，等到sda高电平时才会改变。外接上拉电阻，通信速率和上拉电阻有关。RTC实验的代码还未修改完，未理解代码。



2026年2月6日

今天晚上看了，iic是通过给EEPROM中写数据，然后读数据。实验利用STM32的I2C接口向EEPROM（AT24C02）写入数据（0~255），然后读取出来进行校验，并通过串口将结果打印到电脑上 。模式配置：本实验中 STM32 作为 主发送器（Master Transmitter） 和 主接收器（Master Receiver） 。操作方式：采用 查询事件（Polling） 的方式，即通过不断检测状态寄存器（CheckEvent）来判断当前总线状态（如起始位发送完成、地址发送完成等）。

2026年2月7日
进行iic代码的尝试，发现中间systick的代码还是不知道如何去写，复习首先选择时钟源，进行一个分频的操作，然后将CTRL这个寄存器置一，开启中断当计时器完成时会产生一个中断信号，最后关闭定时器，在初始化中先关闭的原因是配置好参数在启动，防止初始化产生意外的延时。然后就是iic通信的代码操作。 

2026年2月8日

EEPROM 是一种非易失性半导体存储器，全称是 Electrically Erasable Programmable Read‑Only Memory，中文叫“电可擦可编程只读存储器”。EEPROM 在断电后数据不会丢失，属于非易失性存储器，用来长期保存少量重要数据。
了解EEPROM相关的信息，理解代码，准备通过iic来传输相关的数据。



2026年2月9日

今天晚上理解iic传输数据的时序图，并将起始信号与结束信号写出来，还有应答信号。阅读时序图进行理解并尝试编写代码。



```
开始信号(主设备)
1.SDA与SCL都处于高电平,空闲状态
2.发送I2C开始信号:当SCL处于高电平的时候,SDA由高电平变为低电平,开始信号
结束信号：
1.主设备准备发送信号
2.主设备发送结束信号
主设备如果接受从设备的数据,给人家回复应答：
1.主设备”要“发送应答信号
2.准备应答信号
3.主设备发送应答信号
4应答信号结束。
...
1.主设备向外串行发送一个字节，行串行通信一位一位的发的,高位先行。主设备接受从设备发过来的数据，首先定义一个变量，接收从设备发送过来的数据，释放总线,数据线交给从设备，接受来自于从设备的数据。
2.接收数据时，主机或从机都是在 SCL 的上升沿或高电平区间采样 SDA，因此必须保证这段时间 SDA 不变；而在 SCL 低电平 时改变 SDA，才是正确发送下一位数据的时机。



2026年2月11日

昨天未把iic传输数据时等待应答信号给写出，今天晚上完成。还有就是理解EEPROM这个非易失性存储器，断电不丢失，了解页写与片写。向EEPROM写入一个字节的方法：
主机通过广播找到从机的地址，等待从机的应答，主机并发送写入从机的地址，主机发送写入的信号，然后发送写入的数据，最后主设备发送结束信号。



2026年2月25日

复习了EEPROM数据传输的相关内容，与工作中用逻辑分析仪抓到的数据结合相关代码后发现。

在工作中是，读取固定寄存器地址的值，由于工作的代码放到的是一个数组中，然后没读取一个值指针就会向后移动一个单位，直至数据读取完成，从新读取下一组的数据，而这个是EEPROM的地址固定，从这个里面读取相关的数据。

2026年2月26日

单片机通过 IIC 从 EEPROM 读数据时，主机先发起起始信号和“设备地址 + 写”，再发送要读取的存储单元地址，相当于先告诉 EEPROM“我要读第几号位置”；然后主机再次发送起始信号，这次发“设备地址 + 读”进入读模式，EEPROM 就会把该地址里的数据通过 IIC 线发出来，主机接收这个字节后给一个非应答（表示只读这一字节），最后发送停止信号结束本次读操作。

2026年2月27日

单片机通过 IIC 向 EEPROM 写入一页数据时，先发送起始信号，再发‘设备地址 + 写’，等待从机应答；接着发送要写入的起始存储单元地址，收到应答后，用一个 for 循环把这一页的数据一个字节一个字节发给 EEPROM，每发完 1 字节就等待一次应答，全部数据发送完成后，主机发送停止信号结束本次写操作，最后延时约 5ms，等待 EEPROM 在内部完成实际的写入。 

2026年2月28日

今天把代码进行一个完善，编译出错，原因是中间的函数写拼写的有错误。还有就是路径有错误，没有添加正确。先用 Dri_EEPROM_Init 初始化 IIC 接口，然后提供三类读写函数：Dri_EEPROM_WriteByte 会先发送起始信号、设备地址（写）、目标字节地址，再发送要写入的数据字节，收到每步应答后在最后发停止信号，并延时 5ms 等待 EEPROM 完成内部写入；Dri_EEPROM_ReadByte 则先以写模式发送设备地址和要读的存储地址（伪写，用来设置内部地址指针），再重新发起起始信号切换为读模式，发送设备地址（读），收到应答后读回一个字节并发送非应答结束

2026年3月2日

今天晚上将EEPROM接受数据的完成了，理解为啥要加for循环了，因为接受收数据是一个一个字节的数据，通过判断是否接收到最后一个数据，来发送结束信号。还有就是将昨晚的错误修改完毕了，明天将串口打印加上尝试独立编写。



2026年3月3日

今天晚上独立写了串口驱动的初始化函数，发现自己不知道如何去写，需要去复习啊一下前面的内容。



2026年3月4日

编程注意事项，两引脚接收的电平不一样，在TX设置输出的模式为复用推完输出，RX设置输入是上下拉或者浮空输入。还有一点就是硬件控制流应该关闭，停止位是一位， 不使用奇偶校验位，模式设置收发模式都打开， 最后打开使能位否则前面的配置都是无用的。

2026年3月4日

今天复习，一个整数 value 按十进制转换成对应的 C 字符串表示，存到的 string 缓冲区里，并返回这个字符串指针。这个 itoa 函数只处理十进制，把一个 int 转成以 \0 结尾的字符串：如果 radix 不是 10 就直接返回空串；如果数值是 0，就特殊处理生成字符串 "0"；若是负数，先在结果前写入 '-' 并把数值取相反数变为正，再从 10000 开始依次用 1000、100、10、1 去除当前值，算出每一位数字 d，遇到第一位非 0 后开始把 d + '0' 写入字符串，同时从 value 中减掉这位的值以便继续处理下一位，最终在末尾补上 '\0' 返回。在串口打印时候使用。



2026年3月7日

今天看了力扣的第三题，用的是滑动窗口。但是还是不理解题目描述的意思，接着就是完成串口数据的部分程序。

2026年3月7日
今天晚上基本上将串口打印调通，准备通过iic读写数据到eeprom中，准备通过逻辑分析仪抓住波形并进一个分析。



2026年3月15日

今天在独立编程时，发现一个致命性错误，就是在for(初始化语句；判断条件；增量)。初始化在每次程序开始执行一次，能不能进入循环看这个判断条件，循环体跑完才开始执行这个增量。还有就是串口打印的时候，代码在自定义的printf中错误将usart1传入进去，应该是用usartx，因为我在上面的定义就是用usart来的，修改后串口可以将数据进行一个打印。还有就是用逻辑分析仪抓到了串口的数据，并进行一个分析。 



2026年3月25日

还未将代码进行一个调通，今天重新将vscode和keil软件安装好，并用git将远程的代码拉去下来，进行一个分析，首先将数据通过iic写入到eeprom中，然后通过串口打印到写入的数据进行一个打印。
理解写与读的时序图，读可以读取多个字节，而写的话需要根据存储器的每页存储的大小来决定，如何超出则就会从头开始读写。



2026年3月26日

  今天学习spi通信协议，首先这个协议的内容是串行同步的协议，由一主多从设备组成主设备启动一个与从设备的同步通讯，从而完成数据的交换。stm32 的SPI 可以工作在全双工，单向发送，单向接收模式，可以使用 DMA 方式操作。

2026年3月27日

​	编写串口函数，并将串口调通方便后续的代码调试，并完部分的eeprom的写数据。

2026年3月28日

​	完成eeprom的读写，将spi的初始化完成。开启 SPI1、GPIOA、GPIOC 时钟，再配置引脚模式：PC13 设为通用推挽输出作为 CS 片选，PA5 (SCK)、PA7 (MOSI) 为复用推挽输出，PA6 (MISO) 为浮空输入；随后配置 SPI1 为主机模式，采用软件管理 NSS 片选，设为 SPI 模式 0，波特率 8 分频，8 位数据帧、高位先行，最后使能 SPI 外设，同时封装了片选高低电平控制函数和 SPI 单字节收发函数，通过等待发送 / 接收缓冲区状态实现字节的同步收发。

2026年3月29日

​	编写初始化、基础控制类函数：包含调用 SPI 初始化完成芯片底层通信配置，实现读取芯片厂商 / 设备 ID 函数，编写等待芯片空闲（检测 BUSY 位）、写使能、写失能核心辅助函数，掌握 W25Q32 的 SPI 通信时序和基础操作规范，为后续读写擦除功能打基础。


2026年4月3日
使用按键来转换装态，将启动文件放到一个文件夹，跟以前的有所不同，准备用按键做一个状态机，首先对于整个进行一个模块化分层。分为公共层，用来存放打印函数。drivers驱动层，用来存放systick，USART。interface硬件接口用来放按键。user层用来放业务逻辑代码，最后main主函数调用相关逻辑函数。首先尝试把串口打印给调通。

2026年4月4日
准备将LED写好后，准备加入定时器，实现LED呼吸灯，准备用PWM来控制。

2026年4月5日
 今天将按键的完成，并进行调试。在编写代码的时候发现未添加，标准库导致无法识别。   
...
2026年4月8日
今天晚上将代码进行一个调通，将led灯进行一个呼吸灯，通过改变占空比来调节亮度。

今天就能晚上，学习了定时器，定时器分为基本定时器，适用于简单的定时：通用定时器增加了PWM的输出、输入和输出捕获和编码器的接口；高级定时器有互补输出死区控制。初始化操作：“开时钟→配时基→使能中断→配NVIC→启动定时器”的固定流程，核心是通过 TIM_TimeBaseInit设置 预分频器（PSC）​ 和 自动重装值（ARR）​ 来确定定时周期，并配合 TIM_ITConfig与 NVIC 配置实现精准的中断触发。第三步开更新中断。如果只需要定时触发任务，就要打开中断，让定时器时间到了能通知 CPU，不然 CPU 不知道定时结束。第四步配置 NVIC 中断优先级。单片机可能同时有多个中断，设置优先级就是规定哪个中断先处理，避免冲突混乱。

2026年4月10日
今天将基本定时器进行一个初始化，用的是Time6(还有一个基本定时器是Time7)。首先打开时钟，第二步配置时基单元。定时器要定时，必须告诉它多久溢出一次。预分频器用来把高速系统时钟降下来，自动重装载值决定计数到多少归零，两个参数一起算出定时时间。同时设置向上计数模式，这是基本定时器唯一的工作方式。最后在所有参数启动完成后，就要启动定时器。

2026年4月14日

今天晚上通用通定时器，定时1秒，然后打印。初始化完成了，但是vscode无法跳转到对应的函数。

2026年4月15日
tim2定时1s，注意外部引用这个地方，需要设置标志位在主函数中判断在进行清零，串口打印有乱码还未处理。

2026年4月16日

今天晚上尝试理解tim2输出PWM波形来控制LED灯，还有就是了解一下ABD调试。

2026年4月19日

今天发现之前写的标准库代码出现好多错误，原因是函数名没有写对。

力扣寻找有序数组的中位数:首先设置最大最小值函数，目的是找出最大值与最小值，两个数组分别划分为左右两部分，使得左半部分的最大值不大于右半部分的最小值，且左右两边的元素数量相等或左半边多一个，其中要用到二分查找。



2026年4月20日

用定时器来控制led的亮度，在定时器初始化的时候，将led相连的那个引脚的输出模式改为复用涂完模式，输出的频率50MHZ，注意需要开启对应的时钟。，

2026年4月21日

完成了昨天的代码，但是在实践的时候发现未实现led灯亮起，正在排查错误。

2026年4月21日

在排查昨天的错误，复位后等一闪就灭证明led配置是正确，首先去初始化里面进行一个分析，还是未找到问题，准备重新构建初始化进行一个尝试。



2026年4月23日

今天用Tim1定时器进行一个尝试，tim2这个初始化应该是没有写好，还在找为啥不能进行一个PWM控制LED灯的点亮。

2026年4月24日
今天参考之前写的Tim5定时器，尝试用Tim5来控制LED灯的亮度，改成便准库但是LED灯未亮起,还在排查原因，还有就是在调试代码的时候要把keil中文文件关闭，否则在VS coed中编译时下载时，会出现一些问题，就比如在今天晚上打印函数是没有问题，但是却无法打印，排查了一段时间，发现串口何打印函数都没有问题，但就是无法打印，将vscode关闭后，在keil进行下载烧录代码回复正常。

2026年4月26日
今天找到原因是初始占空比设置错误，设置为100，导致如何修改占空比灯都会亮起，为此专门将从新建立一个工程来进行一个测试。还有就是呼吸灯还未用Tim5定时器实现。

2026年4月27日
虽然找到原因昨晚为啥灯不亮的原因，但是还是效果感觉还是不太明显，还在尝试如何修改让效果更好，主备添加按键来控制led灯。

2026年4月28日
今天晚上加入了按键但是发现按键没有反应，也没有打印信息，还在找原因。

2026年4月29日
今天晚上在按键扫描函数中添加了延时20ms，发现按键可以正常响应，但是打印信息没有正常打印。但是第一组按键是上拉输入，第一组显示正常，第二组有误还在找原因。

2026年4月30日
找到原因了，是因为按键接地时，GPIO配置为上拉输入，常态高、按下低；按键接VCC时，GPIO配置为下拉输入，常态低、按下高。内部电阻提供确定常态电平，按键动作引发电平翻转，MCU通过检测此变化识别按键，避免悬空干扰。因此将1与2组按键设置为上拉输入，3与4组按键设置为下拉输入。

2026年5月1日
今天在按键加减占空比，发现占空比没有改变，原因是在那件逻辑中我的逻辑有错误，导致占空比没有改变。现象按一下按键，数值瞬间飙升（甚至加到了 100 爆表），串口疯狂打印 1 和 0 交替的乱码甚至卡死。根本原因： 并非我的大脑逻辑错了，而是“人类的物理时间”与“单片机的运行速度”产生了严重错位，并且代码中存在一个“无视物理真实状态”的逻辑漏洞。如下：
if (key_release == 1) {
    // 发现按键按下，将 key_release 置为 0 (上锁)
} else {
    key_release = 1; // ❌ 致命陷阱：闭着眼睛强行开锁
}
这个 else 的触发条件仅仅是 key_release == 0，它根本没有去读取 GPIO 引脚的真实电平。当程序在几微秒后执行下一轮循环时，虽然你的手还没松开，但因为上一轮 key_release 被设成了 0，程序直接进入 else，强行把状态重置为 1。下一轮循环又会因为状态是 1 而重新进入 if。最终导致：只要手不松，单片机就在 if 和 else 之间以光速反复横跳，疯狂触发有效按键。
解决办法：在else后面加入判断通过判断引脚电平是够改变来判断是否是按键按下。
else if (!is_pressed) {
    key_release = 1; // 必须亲眼看到引脚电平恢复了（手真松开了），才准把状态复位！
}
这个逻辑还有点蒙，还有就是之前为了猜测是否是按键抖动在while中加了按键扫秒的死循环，这个是没有作用的。


2026年5月4日
今天在建立标准库的时候，选错了芯片导致在编译的时候一直报错，但是vscode的插件却编译成功这个不是很理解，SysTick中添加了us延时计数器，ms延时计数器，将打印函数也添加了。
...
2026年5月5日
今天在调串口的时候无法调通，先核对波特率是否正确，发现是正确的，然后原理图看引脚是否定义的正确，但是打印的乱码。

2026年5月6日
首先测量了这两个引脚的电压，发现都是a9是1V左右，a10是3.3V左右，但是逻辑分析没有抓到数据。

2026年5月7日
今天用之前的代码进行一个替换，发现没有打印，但是用之前的程序可以，因该是配置问题。

2026年5月10日
今天加入adc初始化的代码，并准备用可以调节电阻来控制led灯的亮度。

2026年5月11日
找到原因了，是因为再重写fputc的时候没有正确的将fputc函数写入导致错误。
...
2026年5月13日
今天将准备将可调节电压的值打印出来，还未打印成功。

2026年5月16日
今天通过STM32与触摸芯片进行一个通信，接受到触摸之后，中断脚拉低的中断信号，成功接收到但是为正确读取坐标信息。

2026年5月17日
今天将打印出调节电压的值，发现可以正常打印，但是拧动旋钮数值变化几乎没有变化，和之前的寄存器版本相比有很大差异。
...
2026年5月24日
今天将base time的代码进行一个编写，准备用base time来定时。通过按键来切换LED 的工作模式。定时器首先，先开启时钟配置时基，清除中断标志位，配置中断，使能定时器，配置NVIC启动定时器，核心公式：T=(PSC+1)×(ARR+1)/TIM_CLK，需注意PSC/ARR从0计数，16位上限65535。明天准备调试这个模块。

2026年5月25日
今天在调试这个模块时发现，sysTick的值未打印，原因是变量systick未进行一个自增的操作，导致TIM6_IRQHandler()里只清中断，没有 sysTick++，所以 while(1)里的条件永远不成立。然后写了通用定时器的代码，准备控制led灯的亮度。

2026年5月25日
今天调通了adc控制的led亮度，准备添加应用层的代码，来控制led灯的亮度，通过旋转旋钮来控制led灯的亮度。

2026年5月25日

今天还在寻找为啥不能控制亮度的原因，再看引脚是否冲突。

2026年5月29日

今天将代码写完，需要添加滤波处理来解决电压转换的不稳定性问题，上网查资料应该是用滑动窗口滤波来实现。

2026年5月31日
滑动窗口滤波，首先设置窗口的大小。在窗口未满的情况下，将新采样值存入数组当前位置，数组索引递增，指向下一个位置，已存储的样本数递增。在窗口已满的情况下，从总和中减去最旧的值（即将被覆盖的值），用新值覆盖最旧的值，将新值加到总和中，移动索引到下一个位置，如果索引超过窗口大小，重置为0，形成环形缓冲区。
在数据处理中注意C语言的写法这样表述不对如1.5f< v12 < 2.8f,应该是1.5f< v12 && v12 < 2.8f。
   
2026年6月1日
加入按键的代码进行一个修改，学习使用回掉函数来处理按键的状态。

2026年6月2日
添加两个枚举分别是，按键的状态和按键的ID，准备通过定时器来扫描按键的状态，按键按下来触发这个回调函数。

2026年6月3日
使用定时器来计算按键按下的时间未成功，还在修改中。

2026年6月4日
还是未找到原因，打印了Get_TickMs()值，还在分析是原因。

2026年6月7日
找到原因了，是按键初始化未选对对应的引脚，导致按键的状态不能正常触发。然后短按功能实现了，但是长按功能未实现。还在找原因，然后准备用结构体来实现。

2026年6月9日
今天晚上发现串口打印无法打印中文，已经重写了fputc但是keil编译出错，还在找原因为啥不能打印中文。

2026年6月10日
今天遇到一个问题，就是在公司用stm32单片机iic读取数据时，可以打印但是无法正确读取数据，跟据寄存器手册，按照开始时主控先去发送指令，然后去读相应的寄存器，读完后在发送一个结束指令。单TP可以拉中断，通知主控读取数据，准备重写iic之前是移植别的代码，今天把iic的初始化和发送完成。

2026年6月11日
今天在加了按键高低电平的枚举，重新写入了按键配置的结构体，扫描函数重新写了。
